...
def parse_message_with_openai(message_text: str,
   ...
   Если сообщение выглядит как обычное общение, вопрос или не содержит 
   явных указаний на создание задачи 
   (например, не содержит слов "задача", "создай", "поставь", "сделай"), 
   верни is_task: false.
    2) Если это задача, выдели следующие поля:
       ...
       (каждый пункт – строка);
       - project: из списка доступных проектов выбери наиболее подходящий для 
       данной задачи.
